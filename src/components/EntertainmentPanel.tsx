...
export function EntertainmentPanel() {
  ...
  const [scriptName, setScriptName] = useState<string>("entertainment_assistant.py");
  ...
  const run = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setScriptName(data.script || "entertainment_assistant.py");
      } else {
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold" style={{ color: "#a855f7" }}>🎬 Entertainment</h2>
        ...
        <button
          ...
          style={{ backgroundColor: "#a855f7", opacity: loading ? 0.6 : 1 }}
        >
          {loading ? "Running…" : "▶ Run"}
        </button>
      </div>
      ...
      >
        {loading ? "Running script…" : output || "Press Run to fetch entertainment data"}
      </pre>
    </div>
  );
}
