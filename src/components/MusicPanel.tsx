...
export function MusicPanel() {
  ...
  const [scriptName, setScriptName] = useState<string>("music_curator.py");
  ...
  const run = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setScriptName(data.script || "music_curator.py");
      } else {
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold" style={{ color: "#ec4899" }}>🎵 Music</h2>
        ...
        <button
          ...
          style={{ backgroundColor: "#ec4899", opacity: loading ? 0.6 : 1 }}
        >
          {loading ? "Running…" : "▶ Run"}
        </button>
      </div>
      ...
      >
        {loading ? "Running script…" : output || "Press Run to fetch music data"}
      </pre>
    </div>
  );
}
