...
const GRADIENT: [string, string] = ["#3b82f6", "#2563eb"];
const AGENT_NAME = "Hermes-TG";
const BOOT_MSG = "Hermes Telegram thread. Use this for your primary conversation — this stays clean.";
...
export function HermesTelegramPanel() {
  ...
  function handleDrop(e: React.DragEvent) {
    ...
    dropRef.current?.classList.remove("ring-2", "ring-[#3b82f6]");
    attachFiles(e.dataTransfer.files);
  }
  function handleDragOver(e: React.DragEvent) {
    e.preventDefault();
    dropRef.current?.classList.add("ring-2", "ring-[#3b82f6]");
  }
    ...
    <div className="panel flex h-full flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between gap-3 border-b border-white/5 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-base font-semibold tracking-wide text-white">Hermes — Telegram</h2>
            <p className="text-[11px] text-[var(--color-ink-dim)]">Main thread · primary conversation</p>
          </div>
        </div>
            ...
            >
              ...
              {m.content && (hoveredMsg === m.id || tts.speakingId === m.id) && (
                <div className={cn("flex items-end gap-1 pb-0.5", m.role === "user" ? "flex-row-reverse" : "")}>
                  ...
                  {m.role === "assistant" && (
                    <SpeakButton id={m.id} text={m.content} controller={tts} accent="#3b82f6" />
                  )}
                </div>
              )}
            </motion.div>
      ...
      >
        ...
        >
        <AttachMenu accent="#3b82f6" onFile={handleFile} onPaste={handlePaste} onSearch={handleSearch} />
        ...
        <textarea
          ...
          rows={1}
          placeholder={offline ? "Hermes is offline…" : "Message Hermes (main thread)…"}
          className="max-h-[120px] min-h-[44px] flex-1 resize-none rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white outline-none transition placeholder:text-[var(--color-ink-faint)] focus:border-[#3b82f6]/50 focus:bg-black/40"
        />
        {streaming ? (
